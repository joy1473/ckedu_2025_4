...
app.mount("/get_chartHtml_files", StaticFiles(directory="templates/get_chartHtml_files"), name="chart_files")
...
@app.get("/api/render_chart", response_class=HTMLResponse)
async def render_chart():
    try:
        ...
        # Find all matching chart files (htm or html)
        chart_files = glob.glob(os.path.join(templates_dir, "get_chartHtml*.htm*"))
        
        if not chart_files:
            return "<h1>No chart files found</h1>"
            
        # Randomly select one
        selected_file = random.choice(chart_files)
        filename = os.path.basename(selected_file)
        
        with open(selected_file, "r", encoding="utf-8") as f:
            raw_html = f.read()
        
        # Determine likely folder name (e.g. get_chartHtml02.html -> get_chartHtml02_files)
        # Note: get_chartHtml.htm -> get_chartHtml_files (no '01' suffix logic in filename but folder has no suffix? wait)
        # Check specific known mappings or generic logic
        
        # Simplest approach: Replace known patterns
        corrected_html = raw_html
        
        # Mapping for known files
        replacements = [
            ("./get_chartHtml_files/plotly-3.3.1.min.js.다운로드", "/get_chartHtml_files/plotly-3.3.1.min.js")
        ]
        
        for old, new in replacements:
            corrected_html = corrected_html.replace(old, new)
            
        return corrected_html
    except Exception as e:
        return f"<h1>Error loading chart</h1><p>{str(e)}</p>"

@app.post("/chat")
async def chat_endpoint(request: ChatRequest):
    term = request.message
    
    # [Start] Test Chart Injection Logic
    chart_html = ""
    if "분석" in term:
        # Use iframe to ensure scripts run correctly
        chart_html = '<iframe src="/api/render_chart" style="width: 100%; height: 680px; border: none; overflow: hidden; margin-top: 30px;" scrolling="no"></iframe>'
    # [End] Test Chart Injection Logic
    ...
    # Append Chart HTML if it exists
    if chart_html:
        reply_text += f"\n\n{chart_html}"
