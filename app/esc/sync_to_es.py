...
# 1. DB 연결 설정
# MongoDB (config.py에서 가져옴)
# Elasticsearch (비밀번호가 없는 현재 상태 기준)
# es = Elasticsearch([os.getenv("ES_URL", "http://localhost:9200")])
es = Elasticsearch(
    [os.getenv("OPENSEARCH_URL", "https://ckedu-esc-117549366-08ee-pub.kr.ses.naverncp.com")],
    http_auth=(os.getenv("OPENSEARCH_USER", "admin"), os.getenv("OPENSEARCH_PASS", "Xmas1225!")),
