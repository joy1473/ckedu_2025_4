document.addEventListener('DOMContentLoaded', () => {
    ...
    // Update greeting with current user
    function updateGreeting() {
        const greetingElement = document.querySelector('.empty-state h2');
        if (greetingElement) {
            greetingElement.textContent = `${currentUser}님, 안녕하세요!`;
        }
    }
    ...
    async function handleSendMessage() {
        ...
        try {
            const response = await fetch('/emo/chat', {
                method: 'POST',
