...
# FastAPI 앱 생성
APP_QA = FastAPI(title="QA API", version="1.0.0")
...
@APP_QA.get("/qa/status")
def get_qa_status():
    """
    QA 봇 상태 확인
    """
    return {"status": "QA Bot is running", "conversations": len(user_conversations)}

@APP_QA.post("/qa/start")
def start_qa_bot():
    """
    QA 봇 시작 (백그라운드 실행)
    """
    # Discord 봇 시작 (실제로는 별도 스레드에서 실행)
    # bot.run(DISCORD_TOKEN)
    return {"message": "QA Bot started"}
