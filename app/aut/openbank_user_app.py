...
app = FastAPI(title="OpenBank User App", version="1.0.0")
...
async def auth_callback(code: str = None, state: str = None, scope: str = None, error: str = None, error_description: str = None):
  """
  ...
  if not code:
    ...
    return templates.TemplateResponse("auth_result.html", {"request": {}, "result": result_data['result'], "message": result_data['message']})
  ...
  return templates.TemplateResponse("auth_result.html", {"request": {}, "data": result_data})

# 사용자계좌등록 (자체인증 이용기관용)
@app.post("/user/register/")
async def user_register(access_token: str, register_account_num: str, register_account_seq: str,
                  user_info: str, user_name: str, user_ci: str, user_email: str):
