...
# FastAPI 앱 생성
APP_TELEGRAM = FastAPI(title="Telegram API", version="1.0.0")
...
@APP_TELEGRAM.get("/telegram/status")
def get_telegram_status():
    """
    Telegram 봇 상태 확인
    """
    return {"status": "Telegram Bot is running", "known_chats": len(known_chat_ids)}

@APP_TELEGRAM.post("/telegram/send/{message}")
def send_telegram_message(message: str):
    """
    모든 사용자에게 메시지 전송
    """
    send_alarm_to_all(message)
    return {"message": f"Sent to {len(known_chat_ids)} users"}
